...
exports.initOrResumeTrecvidTaskSession = function (sess, viewData) {
  ...
  if (typeof sess.ranker.trecvidSession === "undefined") {
    global.logger.log(
      "debug",
      "Session " + sess.id + " has no active TRECVID task session."
    );
  ...
  // Search session running
    ...
    global.logger.log(
      ...
      "Session " +
        sess.id +
        " has active TRECVID task session: " +
        JSON.stringify(sess.trecvidSession, undefined, 4)
    );
    global.logger.log(
      "debug",
      "with model settings: " +
  ...
  viewData.ranker.settings.aggregationModelSimple = rankerUtils.getModelNumberNameFromSession(
    sess
  );
...
exports.startTrecvidTaskSession = function (sess) {
  ...
  global.logger.log(
    "debug",
    "Starting TRECVID task for session id " +
      sess.id +
      " with ID " +
...
exports.startTrecvidRun = function (sess, tasksFilepath, runType) {
  ...
  sess.trecvidRun.outputFilepath =
    global.gConfig.outputPath +
    String(dateString) +
    "_" +
    String(runType) +
    ".xml";
  ...
  global.logger.log(
    "debug",
    "Starting TRECVID run for session id " + sess.id + "."
  );
...
exports.terminateTrecvidRun = function (sess) {
  ...
  global.logger.log(
    "debug",
    "Terminated TRECVID run for session id " + sess.id + "."
  );
...
exports.printXmlHeader = function (sess) {
  ...
  if (sess.ranker.settings.aggregationModel.id == 3) {
    ...
    if (sess.ranker.settings.aggregationModel.viret.queryOperations == 0) {
      ...
      sess.ranker.settings.aggregationModel.viret.queryOperations == 1
    ) {
      descString += "Product + Max";
    } else if (
      sess.ranker.settings.aggregationModel.viret.queryOperations == 2
    ) {
      descString += "Sum + Sum";
    } else if (
      sess.ranker.settings.aggregationModel.viret.queryOperations == 3
    ) {
      descString += "Sum + Max";
    }
  ...
  let headerString =
    ...
    \t<videoAdhocSearchRunResult trType="A" class="M" task="' +
    sess.trecvidRun.runType +
    '" novelty="C" pid="SIRET" priority="1" desc="' +
    descString +
    '">\n';
...
exports.printXmlShots = function (sess, elapsedTime, shots) {
  ...
  let totalString =
    '\t\t <videoAdhocSearchTopicResult tNum="' +
    taskId +
    '" elapsedTime="' +
    elapsedTimeRounded +
    '">\n';
